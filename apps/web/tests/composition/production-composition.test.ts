...
const productionEnvironment = {
  ...
  MEDBUDDY_MEMORY_FORMATION_CALLBACK_URL: "https://fictional.example.test/api/internal/memory-formation",
  MEDBUDDY_PASSIVE_MEMORY_CALLBACK_URL: "https://fictional.example.test/api/internal/passive-memory",
...
describe("production composition configuration", () => {
  ...
  it("requires the approved private task, storage, and Gemini settings without echoing values", () => {
    expect(loadContinuityConfiguration(productionEnvironment)).toEqual({
      ...
      memoryFormationCallbackUrl: "https://fictional.example.test/api/internal/memory-formation",
      passiveMemoryCallbackUrl: "https://fictional.example.test/api/internal/passive-memory",
