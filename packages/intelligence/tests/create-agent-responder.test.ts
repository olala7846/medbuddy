  ...
  it("executes an authorized family-map replacement and returns the model acknowledgment", async () => {
    const model = fakeModel()
      .respondWithTools([{
        ...
        args: {
          expectedRevision: 0,
          content: "Participants\n- Mei (member:fictional-caregiver)\n\nNamed relatives\n\nDirect relationships",
        },
        id: "call:family-map",
      }])
    ...
    expect(model.calls[0]?.messages[0]?.text).toContain("copy the complete opaque ID byte-for-byte");
    expect(model.calls[0]?.messages[0]?.text).toContain("Participants, Named relatives, Direct relationships");
    expect(model.calls[0]?.messages.at(-1)?.text).toBe(
      "[author:member:fictional-caregiver]\nI am Mei.",
    );
