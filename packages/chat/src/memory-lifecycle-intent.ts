...
export function classifyExplicitMemoryLifecycle(bodyValue: string): ExplicitMemoryLifecycleAction | null {
  ...
  if (/\b(?:if|whether|wonder|maybe|perhaps|possibly|might|could|would|unsure|uncertain|suppose|think|guess|seems?|apparently)\b|(?:如果|假如|若|萬一|要是|也許|或許|可能|不確定|是不是|我想|我猜|似乎|好像)/iu.test(body)) {
    return null;
  }
  if (/[：:]\s*(?:is|are|was|were|do|does|did|can|could|would|should|will|has|have)\b|[：:]\s*(?:是否|是不是|有沒有)/iu.test(body)) return null;
  if (/["“”「」『』]/u.test(body) ||
      /\b(?:said|says|asked|asks|told|quoted|reported|reports|heard|hears|claims?)\b|(?:她說|他說|有人說|轉述|聽說|表示|聲稱|要求)/iu.test(body)) {
    return null;
  }
